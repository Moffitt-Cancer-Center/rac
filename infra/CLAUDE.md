...
- `modules/pipeline-kv.bicep` — dedicated Key Vault `kv-rac-pl-${env}` for per-submission HMAC callback secrets. **Public network access is intentionally enabled** (divergence from the platform KV) so the rac-pipeline GHA runner can read secrets without VNet peering. Strict 24-char Azure naming limit applies — the module name pattern is abbreviated `kv-rac-pl-${env}` to fit. Gated by `deployPipelineKv` (default false).
